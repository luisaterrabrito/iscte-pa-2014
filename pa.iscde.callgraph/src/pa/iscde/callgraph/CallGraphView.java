 ...
 *
 */
public class CallGraphView implements PidescoView {
	...
	private Graph graph;
	private static CallGraphView instance;
	...
	boolean selectingMethod = false;
	
	/**
	 * 
	 * Retrieves CallGraph icon Image.
	 * 
	 * @return CallGraph Image
	 */
	public Image getCallGraphIcon(){
		return callGraphIcon;
	}
	 ...
	 */
	public static CallGraphView getInstance() {
		return instance;
	}
	...
	public CallGraphView() {
		Bundle bundle = FrameworkUtil.getBundle(CallGraphView.class);
	 ...
	 */
	private void loadExportButtons(Shell shell){
...
//		clearGraph(graph);
		...
		ASTVisitor visitor = new ASTVisitor() {
			...
			public boolean visit(MethodDeclaration node) {
				...
				if(!selectingMethod) {
					...
					currentMethod = getMethodSelected();
				}
				activeMethod = node;
				invocationsForMethods.put(activeMethod, new ArrayList<MethodInvocation>());
				return super.visit(node);
		...
		drawDiagram(currentMethod);
	}

	public void drawDiagram(MethodDeclaration md) {
...
//					if(key == md) {
					if(key.getName().toString().equals(md.getName().toString())) {
						...
						for (HashMap.Entry<String, HashMap<MethodDeclaration, ArrayList<MethodInvocation>>> entry : classMethods.entrySet()) {
							...
							for (HashMap.Entry<MethodDeclaration, ArrayList<MethodInvocation>> entry2 : entry.getValue().entrySet()) {
								...
								for (MethodInvocation mi2 : classMethods.get(entry.getKey()).get(entry2.getKey())) {
									...
									// classe que pertence
									if(mi2.getName().toString().equals(md.getName().toString())) {
	...
	private MethodDeclaration getMethodSelected() {
		return selectedMethod;
	}
	
	private void setMethodSelected(MethodDeclaration m) {
		selectedMethod = m;
	}
	...
	public void selectMethod(final ASTNode node) {
		MethodDeclaration selected = (MethodDeclaration) node;
